...
**Response**: Added comma after H and \ in the punctuation.
...
**Response**:Since they were different length of columns it will give us a warming that it will not turn out well
...
```{r}
len <- length(Symbol$my_symbols)
len
```

Comment on what you noticed about the errors and how you used this information to correct the issues.

**Response**: Needed to add a dollar sign to show where the data is coming from
...
**Response**: The % sign does not connect the two values you would need a $.
...
```{r}
#step 5
top_secret <- matrix(newmsg,ncol=5)
#Step 6
evens <- seq(2,502,2)
#evens
#Step 7
